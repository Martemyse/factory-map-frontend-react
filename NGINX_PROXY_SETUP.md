...
This application uses **Nginx as a reverse proxy** to handle all external traffic through a single port (8087). This provides better security and simplifies firewall configuration.

### Port Configuration

| Service | Internal Port | External Port | Access |
|---------|--------------|---------------|---------|
| Frontend (Nginx) | 8087 | 8087 | ✅ Public |
...
```
Browser → http://ecotech.utlth-ol.si:8087/
...
### ✅ Advantages
1. **Single Port**: Only port 8087 needs to be open in firewall
...
- Required ports 8087, 7998, 7999 all open
...
services:
  ...
  # Frontend - Nginx with port 8087
  react_app_factory_map_frontend:
    ports:
      - "8087:8087"  # Only port exposed to host
...
Only port 8087 needs to be open:
...
When you open `http://ecotech.utlth-ol.si:8087/`, the console should show:
...
1. `http://ecotech.utlth-ol.si:8087/` → 200 (HTML)
2. `http://ecotech.utlth-ol.si:8087/assets/index-XXX.js` → 200 (JS)
3. `http://ecotech.utlth-ol.si:8087/assets/index-XXX.css` → 200 (CSS)
4. `http://ecotech.utlth-ol.si:8087/api/features/geojson` → 200 (API)
5. `http://ecotech.utlth-ol.si:8087/tiles/data/LTH_factory/{z}/{x}/{y}.pbf` → 200/204 (Tiles)

**All requests go to port 8087** - Nginx handles the routing internally!
...
### Production (Linux)
- Everything: `http://ecotech.utlth-ol.si:8087`
- Only port 8087 exposed
